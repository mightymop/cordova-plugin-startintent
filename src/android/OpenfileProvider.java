...
public class OpenfileProvider extends FileProvider {
  ...
  public int delete(@NonNull Uri uri, @Nullable String selection, @Nullable String[] selectionArgs) {
    ...
    String path = java.net.URLDecoder.decode(uri.getPath());
    pathCheck();

    File ffile = !translatepath_internal ? new File(getContext().getFilesDir(), path) : new File(internal + "/files", path);
    Log.i(pluginName, "TEST: " + ffile.getAbsolutePath());
    if (!ffile.exists()) {
      ffile = !translatepath_internal ? new File(getContext().getCacheDir(), path) : new File(internal + "/cache", path);
      Log.i(pluginName, "TEST: " + ffile.getAbsolutePath());
      if (!ffile.exists()) {
        ffile = !translatepath_internal ? new File(getContext().getDataDir(), path) : new File(internal, path);
        Log.i(pluginName, "TEST: " + ffile.getAbsolutePath());
        if (!ffile.exists()) {
          ffile = !translatepath_external ? new File(getContext().getExternalFilesDir(null), path) : new File(external + "/files", path);
          Log.i(pluginName, "TEST: " + ffile.getAbsolutePath());
          if (!ffile.exists()) {
            ffile = !translatepath_external ? new File(getContext().getExternalCacheDir(), path) : new File(external + "/cache", path);
            Log.i(pluginName, "TEST: " + ffile.getAbsolutePath());
            if (!ffile.exists()) {
              Log.e(pluginName, "getUri: file (" + path + ") does not exist");
              return 0;
            }
          }
        }
  ...
  public ParcelFileDescriptor openFile(@NonNull Uri uri, @NonNull String mode) throws FileNotFoundException {
    ...
    String uristr = java.net.URLDecoder.decode(uri.toString());
    File fname = null;
    ...
    if (uristr.contains(getProviderURI() + "/internal/")) {
      ...
      fname = !translatepath_internal ? new File(getContext().getFilesDir(), uristr) : new File(internal + "/files", uristr);
      new File(getContext().getFilesDir(), "files/" + uristr);
    } else if (uristr.contains(getProviderURI() + "/external/")) {
      uristr = uristr.replace(getProviderURI() + "/external/", "");
      fname = new File(Environment.getExternalStorageDirectory(), uristr);
    } else if (uristr.contains(getProviderURI() + "/external2/")) {
    ...
    } else if (uristr.contains(getProviderURI() + "/cache/")) {
    ...
    } else if (uristr.contains(getProviderURI() + "/cache2/")) {
    ...
    } else if (mode.equalsIgnoreCase("rt") || mode.equalsIgnoreCase("tr")) {
      imode |= ParcelFileDescriptor.MODE_READ_ONLY | ParcelFileDescriptor.MODE_TRUNCATE;
    } else if (mode.toLowerCase().contains("rw") || mode.toLowerCase().contains("wr")) {
